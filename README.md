...
| **河南科技大学校园网直连（大学掌体系）** | 直连渠道（免浏览器 / Python）：先取 CSRF 令牌再 POST 登录，令牌绑定连接 |

> 带「直连」的条目是**直连任务**（`type: "http"`），在「任务 → 直连任务」页的「仓库导入」里出现；其余是浏览器任务，在「浏览器任务」页导入。索引条目按 `type` 分流，缺省视为 `browser`（老条目因此不受影响）。
...
> 索引条目还有两个可选字段：`type`（`browser` / `http`，缺省 `browser`；直连任务必须写 `"http"`，否则不会出现在「直连任务」页的导入列表里）与 `source`（**来源仓库地址**，任务改编自他人脚本/项目时标注出处，导入弹窗的详情区会渲染成可点开的链接；仅接受 http/https）。
